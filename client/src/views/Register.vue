  ...
  <div>
    <form class="border" @submit.prevent="">
      ...
      <div class="input">
        <div class="box">
          <label class="label">
            <span v-bind:class="{'in' : username}" >Username</span>
            <input class="test" v-bind:class="{'not-empty' : username}" type="text" v-model="username">
          </label>
        </div>
      </div>
      ...
      <div class="input">
        <div class="box">
          <label class="label">
            <span v-bind:class="{'in' : password2}" >Confirm Password</span>
            <input class="test" v-bind:class="{'not-empty' : password2}" type="password" v-model="password2">
          </label>
        </div>
      </div>
      ...
      <div>
        <button :disabled="!email.match(/^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/) || username.length <= 0 || password.length < 6 || password2.length < 6 || password !== password2"  @click="register">
          Login
        </button>
      </div>
    </form>
...
export default {
  ...
  methods : {
    async register() {
      ...
      try{
        let searchUser = await axios.get(`http://localhost:3000/user/${this.username}`)
        console.log(searchUser)
        if(searchUser.data === true){
          ...
          this.$store.commit('setUser', user.data.result)
          this.$router.push('/')
        }else{
          this.errorMessage = searchUser.data
        }
      } catch (error){
        console.log(error)
